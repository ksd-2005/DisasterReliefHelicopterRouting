...
double compute_objective(const Solution &sol, const ProblemData &data,
	...
	for (const auto &plan : sol) {
		...
		if (heli_total_dist > heli.distance_capacity) {
			is_valid = false;
		}
	}
...
Solution reorder_visits(const Solution &HelicopterPlans,
	...
	for (int attempt = 0; attempt < 10; ++attempt) {
		...
		if (new_dist > problemData.d_max ||
			new_heli_total > heli.distance_capacity) {
		...
		if (delta_cost < 0) { // Better objective (lower cost)
...
Solution move_visit(const Solution &HelicopterPlans,
	...
	for (int attempt = 0; attempt < 10; ++attempt) {
		...
		if (sourceIt->tripIndex == targetIt->tripIndex) {
			...
			if (tripLoads.size() >= 2) {
				targetIt = std::find_if(tripLoads.begin(), tripLoads.end(),
										[&tripLoads](const TripLoad &load) {
											return load.tripIndex ==
												   tripLoads[1].tripIndex;
										});
			} else {
				continue;
			}
		...
		bool valid = (new_dist_s <= problemData.d_max) &&
					 (new_dist_t <= problemData.d_max) &&
					 (new_heli_total <= heli.distance_capacity) &&
		...
		if (valid) {
			double delta_cost = heli.alpha * delta_dist;
			double obj = current_obj - delta_cost;
			if (obj > current_obj) {
				return movedPlans;
			}
		}
...
Solution split_trip(const Solution &sol, const ProblemData &problemData,
	...
	for (int attempt = 0; attempt < 10; ++attempt) {
		...
		bool valid = (new_old_dist <= problemData.d_max) &&
					 (new_trip_dist <= problemData.d_max) &&
					 (new_heli_total <= heli.distance_capacity) &&
...
Solution merge_trips(const Solution &sol, const ProblemData &problemData,
	...
	for (int attempt = 0; attempt < 10; ++attempt) {
		...
		bool valid = (merged_dist <= problemData.d_max) &&
					 (new_heli_total <= heli.distance_capacity) &&
...
Solution reassign_visit(const Solution &sol, const ProblemData &problemData,
	...
	for (int attempt = 0; attempt < 10; ++attempt) {
		...
		double delta_cost =
			dst_heli.alpha * delta_dist_dst + src_heli.alpha * delta_dist_src;
		...
		bool valid = (compute_trip_dist(srcTrip, src_home, problemData) <=
					  problemData.d_max) &&
					 (new_dst_dist <= problemData.d_max) &&
					 (new_src_heli_total <= src_heli.distance_capacity) &&
					 (new_dst_heli_total <= dst_heli.distance_capacity) &&
...
Solution get_best_neighbor(const Solution &HelicopterPlans,
	...
	update_best(reorder_visits(HelicopterPlans, problemData, current_obj));
	update_best(reallocate_packages(HelicopterPlans, problemData, current_obj));
	update_best(move_visit(HelicopterPlans, problemData, current_obj));
	update_best(split_trip(HelicopterPlans, problemData, current_obj));
	update_best(merge_trips(HelicopterPlans, problemData, current_obj));
	update_best(reassign_visit(HelicopterPlans, problemData, current_obj));
